...
const fallbackExhibition = {
  ...
  description: "Een expositie die onderzoekt hoe mensen zich verhouden tot elkaar en hun omgeving.",
  ...
  return (
    <div className="bg-white">
      ...
      <section className="px-7 lg:px-12 pt-20 lg:pt-28 pb-14 border-b border-border">
        <p className="text-[9px] uppercase tracking-[0.28em] text-muted mb-5">Autonoom werk</p>
      ...
      <section className="px-7 lg:px-12 py-14 lg:py-20 border-b border-border">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-start">
          ...
          <div className="lg:pt-2">
            ...
            <h2 className="text-2xl lg:text-3xl font-light leading-snug text-ink">{expo.title}</h2>
            <p className="mt-5 text-sm text-ink/55 leading-relaxed">{expo.description}</p>
        ...
        <div className="space-y-20 lg:space-y-28">
          {series.map((s: any, i: number) => (
            <div key={s.title} className={`grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-center ${i % 2 === 1 ? "lg:grid-flow-dense" : ""}`}>
              ...
              <div>
                <p className="text-[9px] uppercase tracking-[0.28em] text-faint mb-4">{s.year}</p>
                <h2 className="text-2xl lg:text-3xl font-light leading-tight text-ink">{s.title}</h2>
                <p className="mt-5 text-sm text-ink/55 leading-relaxed">{s.text}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <section className="px-7 lg:px-12 py-10 border-b border-border">
        ...
        {press.map((item: any, i: number) => (
          <a key={i} href={item.url} target="_blank" rel="noopener noreferrer"
            className="group flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 py-6 border-t border-border">
            <div className="flex items-start gap-6">
              <span className="text-[9px] uppercase tracking-[0.22em] text-faint w-12 shrink-0 mt-0.5">{item.publication}</span>
              <div>
                <h3 className="text-sm font-medium leading-snug max-w-xl group-hover:text-ink/60 transition-colors">{item.title}</h3>
                <p className="mt-1.5 text-xs text-ink/40">{item.description}</p>
              </div>
            </div>
            <span className="shrink-0 text-faint group-hover:text-ink group-hover:translate-x-1 inline-block transition-all duration-300">→</span>
          </a>
        ))}
      </section>
      ...
      {/* ── CTA ────────────────────────────────────── */}
      <section className="px-7 lg:px-12 py-14 lg:py-20 border-b border-border">
        <p className="text-[9px] uppercase tracking-[0.28em] text-muted mb-5">Samenwerken</p>
        <h2 className="text-xl lg:text-2xl font-light leading-tight text-ink max-w-md">
